from __future__ import annotations

import sys
from pathlib import Path

import pandas as pd
import pytest
import torch
import yaml

from data_loader import load_data
from scripts import build_3182_dataset
from scripts.build_3182_dataset import (
    CORRECTIONS_PATH,
    PROJECT_ROOT,
    _normalize_predecessor_tokens,
)
from utils.resource_graph import build_task_skill_edges


CANONICAL_DATASETS = {
    str(size): PROJECT_ROOT / "data" / f"{size}.csv"
    for size in (283, 680, 2338, 3182)
}
CSV_PATH = CANONICAL_DATASETS["3182"]


def test_four_canonical_csv_datasets_are_the_training_sources() -> None:
    for name, path in CANONICAL_DATASETS.items():
        assert path.is_file(), f"缺少唯一权威数据集：{path}"
        loaded = load_data(path)["task_df"]
        assert int(loaded["node_type"].eq(2).sum()) == int(name)

    from scripts.generate_multiscale_datasets import choose_template

    expected_templates = {
        1: "283.csv",
        284: "680.csv",
        681: "2338.csv",
        2339: "3182.csv",
    }
    for target_ops, expected_name in expected_templates.items():
        assert choose_template(target_ops) == PROJECT_ROOT / "data" / expected_name


def test_legacy_3182_builder_writes_only_to_candidate_output(monkeypatch) -> None:
    output = pd.DataFrame(
        {"类型": [2], "加工时间/h": [1.0], "需求人数": [1], "工种": [0]}
    )
    output_targets: list[Path] = []
    monkeypatch.setattr(
        build_3182_dataset,
        "build_dataset",
        lambda _config: (output, pd.DataFrame()),
    )
    monkeypatch.setattr(
        build_3182_dataset,
        "write_and_verify",
        lambda _output, _legacy, path: output_targets.append(path),
    )
    monkeypatch.setattr(sys, "argv", [str(build_3182_dataset.__file__)])

    build_3182_dataset.main()

    assert output_targets == [PROJECT_ROOT / "results" / "data_candidates" / "3182.csv"]
    assert output_targets[0] != CANONICAL_DATASETS["3182"]


def test_node_type_and_skill_type_are_independent() -> None:
    loaded = load_data(CSV_PATH)["task_df"]
    physical = loaded["node_type"].eq(2)

    assert len(loaded) == 3299
    assert int(physical.sum()) == 3182
    assert set(loaded.loc[physical, "skill_type"].unique()) == set(range(5))
    assert set(loaded.loc[~physical, "skill_type"].unique()) == {-1}
    assert loaded.loc[physical, "profession_code"].nunique() == 17


def test_registered_predecessor_corrections_are_applied_to_all_target_datasets() -> None:
    payload = yaml.safe_load(CORRECTIONS_PATH.read_text(encoding="utf-8"))
    corrections = payload["corrections"]
    assert corrections

    for correction in corrections:
        for dataset in correction["datasets"]:
            frame = pd.read_csv(PROJECT_ROOT / dataset)
            rows = frame.loc[frame["AO号"].astype(str).eq(correction["task_ao"])]
            assert len(rows) == 1
            predecessors = _normalize_predecessor_tokens(rows.iloc[0]["紧前工序AO号"])
            assert correction["removed_predecessor_ao"] not in predecessors
            assert predecessors == correction["retained_predecessors"]


def test_legacy_csv_without_explicit_skill_schema_is_rejected(tmp_path) -> None:
    legacy_path = tmp_path / "legacy_without_skill_column.csv"
    pd.DataFrame(
        {
            "AO号": ["A", "AA0001-0010"],
            "类型": [1, 2],
            "紧前工序AO号": ["", "A"],
            "需求人数": [0, 2],
            "加工时间/h": [0.0, 1.0],
        }
    ).to_csv(legacy_path, index=False)

    with pytest.raises(ValueError, match="历史无工种 CSV 不可运行"):
        load_data(legacy_path)


def test_virtual_nodes_have_no_skill_hub_edge() -> None:
    # task_x 形状：[3, 18]；前两个物理工序分别需要工种 0 和 4，第三个为虚拟节点。
    task_x = torch.zeros((3, 18))
    task_x[0, 5] = 1.0
    task_x[1, 9] = 1.0
    edges = build_task_skill_edges(task_x, num_skill_types=5)

    assert edges.tolist() == [[0, 4], [0, 1]]

...
class Config:
    ...
    # 站位最小前瞻贪心解码 (MES Decoding: Minimum Earliest Station)
    enable_mes_decoding: bool = False
    mes_penalty_lambda: float = 2.0
    mes_max_station_jump: Optional[int] = 1
    mes_mode: str = "soft_penalty"
    ...
    ddqn_enable_vector_env: bool = True         # 使用平台配置的多进程环境并行采样
    ddqn_num_envs: int = 1                     # DDQN rollout 环境数；平台配置覆盖
    ddqn_enable_ipc_fusion: bool = True        # 仅 DDQN 向量 rollout 使用融合 IPC
    literature_ppo_enable_vector_env: bool = True  # 文献 PPO 复用主方法向量 rollout
    ...
    enable_rollout_ipc_fusion: bool = True       # 将 step/wait 后的 masks 与 snapshot 合并为一次 IPC (默认启用，减少 50% 管道往返)
    rollout_double_buffer: bool = False          # 是否开启 A/B 环境双缓冲流水线 (CPU 步进与 GPU 前向推断重叠)

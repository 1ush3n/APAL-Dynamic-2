...
**日期：** 2026-09-24
**状态：** 架构方向已获用户确认；本文档待用户审阅。尚未修改训练代码，也未启动完整批次试点。
...
环境每次准备好策略决策时，返回不可变 `DecisionSnapshot`：只含CPU可序列化数据，足以复原该时刻图特征、候选任务、留站/后移掩码、工人候选与团队补全掩码、任务/工人节点索引映射、时间与周期标识。快照不能包含活环境引用、CUDA张量或会在环境变化后被改写的对象。Actor以快照采样并生成重放记录；动作回传对应环境进程，由 `env.step()` 作最终权威校验。

采样采用同步向量轮次：每轮收集所有仍活动环境的决策快照，在主进程由同一Actor产生动作，再并行发送给各环境并等待本轮结果。rollout段内不更新策略参数，避免异步worker使用不同策略版本造成PPO样本陈旧。每段总样本数按配置的**聚合交互步数**计算，尾段允许少于 `num_envs` 个活动样本，不通过补造动作凑整。

训练场景按冻结清单预先映射到 `(worker_id, episode_index)`，不按哪个worker先完成来临时抽场景。C/D共享事件清单、训练种子集合和聚合交互预算；每条记录同时报告预设目标数、实际命中数和未命中原因。

达到 `successful_batch_target` 后停止分配新episode；其他已经开始的episode在剩余交互/墙钟预算允许时继续至终态，否则明确记为截断并保留失败记录。不得为了凑成功数而丢弃在途episode结果。

## 5. PPO、标签、塑形和终止语义

- 每个worker/episode独立保存转移和 `PendingTimeLabelCache`；标签键至少区分环境、episode、cycle。标签缓存跨多个rollout段保留到真实转站或轨迹失败/结束，保存CPU图快照、启发式值及采样时显式时间输入，不持有可变环境对象。
- 每个worker的轨迹段单独计算GAE和bootstrap，再合并已完成的样本用于一次策略更新；绝不跨worker、跨episode或跨终止边界串接优势值。真实成功终止与不可恢复失败均不接到下一条reset轨迹；可继续采样的段末保留合法后继价值。预算截断不伪造时间标签。
- 环境的原始奖励及分项账本随对应 `step()` 返回。塑形只使用该次转移的原始奖励和同一冻结预测器版本的前后势函数，不在rollout协调器中重新估算或重复记费。
- 预测器副本按现有确认规则在完整生产轨迹边界刷新；同一episode内固定版本。环境worker不持有或更新预测器。
...
评测用检查点必须严格包含Actor/Critic/图编码器、在线时间头、归一化状态、图特征版本、方法profile、resolved config及散列、源码提交和场景/预测器版本。Lightning训练检查点另保存优化器、学习率状态（若配置）、精度插件状态/GradScaler（FP16时）、Trainer步数、随机状态及事件计划位置。worker当前环境状态若未实现并验证可恢复，检查点必须标为非精确续训，不能宣称位级/轨迹级断点续训；这不影响完整模型评测加载。
...
1. **配置与种子：** rag_env下可加载/校验YAML；配置覆盖能被resolved快照和散列复现；相同种子C/D Actor指纹一致；路径跨Windows/Linux。
2. **环境进程：** 用 `spawn` 启动至少两个环境，固定场景映射稳定；动作/奖励/同刻事件与单环境语义一致；worker异常、超时和关闭无挂起子进程。
3. **快照与PPO：** 同一快照采样与重放动作对数概率满足经FP32/AMP校准的容差；旧环境引用或新状态不能改变重放；GAE不跨worker/episode串接。
4. **AMP与Lightning：** 训练入口可追踪到Lightning Trainer、Work3LightningModule、唯一优化器及AMP上下文；PPO与辅助损失能更新正确共享参数；损失、梯度和日志无NaN/Inf；FP16缩放如启用只有一个所有者。
5. **资源/恢复：** 保存的检查点可严格加载完整推理模型和配置元数据；在RTX 4060 8GB、指定 `num_envs`、batch size、rollout长度和dtype组合下记录显存/主机内存，结论仅适用于该配置。
6. **小预算闭环：** 两环境smoke按聚合步数停止，完成至少一次Lightning优化更新，并逐worker记录成功/失败/截断、实际命中、奖励分项和独立可行性；随后才能冻结完整批次试点协议。小预算smoke不替代完整轨迹或性能结论。

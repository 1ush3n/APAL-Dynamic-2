...
**当前阶段门槛及下一项：** C未完整完成；D首轮因OOM中断，二者均未通过完整批次门槛，不得进入配对费用比较。快照累计迁移缺陷已有代码修复及隔离反例，但完整D是否可稳定运行仍须由原冻结协议全程复跑确认；OOM当时精确上层调用帧未留存，不推断其它瞬时显存因素不存在。现在异常路径worker回收也已通过测试；下一步按原`conf/work3/pilot_trial_20260925.yaml`重跑D，不改预算、事件、场景和C/D定义，运行中记录worker退出、峰值显存、真实命中和终止原因；不得把首轮中断片段当成可续训D检查点。

...
**当前阶段门槛及下一项：** C因聚合step上限截断、D修复后复跑因墙钟上限截断，均未完整完成且独立可行性未评估，不得进入配对费用比较。D复跑越过首轮OOM点并运行至墙钟上限，但固定扰动实际命中仍为0；完整生产能力与受扰效果均未证明。下一步依据C/D报告定位完成速度、策略推进/后移和时间辅助更新耗时，再决定是否需要新的试点预算或运行优化；任何预算、事件或并行设置变化都须先冻结并提交新协议。不可精确续训的D权重不得用作正式检查点；暂不开始正式性能比较、测试集调参或GitHub推送。

...
**异常路径worker清理修复（2026-09-25）：** 新增主进程Actor异常反例，旧入口失败时两个worker均仍存活（`1 failed`）。在`run_training()`入口登记本次调用创建的向量环境，并在正常返回或任何异常传播前统一关闭；原有正常关闭审计仍保留，重复关闭幂等，清理错误日志化而不覆盖训练原异常。异常、worker错误和正常烟测`3 passed in 80.17s`；完整训练入口加单环境执行语义和worker超时清理`8 passed in 317.93s`；`py_compile`、`git diff --check`通过。提交`6efa519`。

**D修复后复跑记录（2026-09-25）：** 严格沿用冻结YAML和事件，不改预算或方法配置。运行5407.938秒后按`wall_time_limit`正常收尾，完成7646个聚合环境step（每个worker 3823步），120次Lightning fit、237次优化器更新；0/2批次成功。两条轨迹完成1854/2830和1859/2830工序、各转站10次，独立可行性均为`incomplete_not_assessed`。两个固定扰动均已触发，但目标`3_27`分别在1180.993和1175.752完工，早于tau=1463.7837，实际命中为0。真实时间标签7203条、时间监督成功更新117次；采样重放最大概率误差为0、AMP跳步为0。按每轮均值加权后的原始奖励合计为-13.329291519146793，独立累计费用为13.32929151914679，差值3.55e-15。峰值GPU分配1750186496字节、主进程RSS 3111075840字节；本轮无OOM，worker退出后均已结束。D报告和权重已以`4d141b3`归档；报告/权重SHA分别为`e7eecaccb0832d5199e7965863cb23c537a9d5398dcf759ea6013c9cf3eb208a`和`a6d463c2fadaeb099a2eb780f01c8f86a411bae6e718c9c8dece55dd0a08cf08`。检查点`checkpoint_evaluation_eligible=false`且`resume_capability=non_exact`，只作追溯。

**只读吞吐诊断：** C整体吞吐为`2.931 env.step/s`；D为`1.414 env.step/s`。D的11个含时间监督更新rollout累计耗时`1376.843s`（约占历史rollout耗时25.6%，覆盖704个环境step）；其余rollout耗时`4009.158s/6942 steps`，约`1.732 step/s`。这些区间包括采样与多种优化开销，不能单独归因于辅助时间损失。按D整体吞吐线性外推12000步约需`8487s`，仅作预算量级参照。两组实际命中均为0，且训练清单前两个事件都针对`3_27`；不能靠事后挑选有利事件补结果。

**主进程短测profile及修复（2026-09-25）：** C/D以相同训练集首个固定事件、seed 42、单spawn环境、BF16及128聚合step执行短测`cProfile`。D旧路径的`compute_online_snapshot_time_inputs`调用258次、Actor前向855次、GATv2前向7695次；主进程`run_training`累计107.11秒。根因是每步提前为正常下一状态计算时间修正特征，但此值仅在环境真实截断时被bootstrap消费；新决策与段尾bootstrap会各自计算所需表征。新增普通/真实截断参数化反例先复现`1 failed, 1 passed`，实现将下一状态修正预测延迟到真实截断分支；定向训练入口、F07和PPO回归`21 passed in 168.77s`，编译和diff检查通过，提交`599a802`。同输入修改后在线预测调用降至130、Actor前向727、GATv2前向6543，`run_training`累计99.04秒；报告耗时106.828→98.750秒。初始Actor、事件、标签、轨迹账本和逐段原始奖励相同。profile含启动开销且未采样spawn worker内部，不外推完整训练性能。

**当前阶段门槛及下一项：** C因聚合step上限截断、D复跑因墙钟上限截断，均未完整完成且独立可行性未评估，不得进入配对费用比较。D已越过首轮OOM点，但固定扰动实际命中仍为0；完整生产能力与受扰效果均未证明。下一项是继续检查spawn环境worker内部耗时及训练集首批事件覆盖设计，再据证据决定是否需要更多运行优化或冻结新试点协议。任何预算、事件或并行设置变化都须先冻结并提交新协议。不可精确续训的D权重不得用作正式检查点；暂不开始正式性能比较、测试集调参或GitHub推送。

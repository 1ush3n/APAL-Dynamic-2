...
### 10.4 四个权威真实数据集与历史重建工具

唯一权威真实任务数据集是本目录中的 `283.csv`、`680.csv`、`2338.csv` 和 `3182.csv`；训练、验证和评测应直接使用这些文件，或使用明确记录由这些文件生成的派生样本。外部 Excel 不作为权威数据源。

`scripts/build_3182_dataset.py` 仅保留为历史映射/转换工具。若配置工作簿存在，脚本默认生成候选文件 `results/data_candidates/3182.csv`，不会覆盖权威的 `data/3182.csv`；候选文件不得直接冒充正式数据或进入训练清单。

运行环境中的五技能映射仍保存在 `conf/data/skill_groups_5.yaml`。其中工作簿只作为可选候选转换输入，五类分组是实验性资源建模假设，不等同于工厂确认的专业互换关系。正式训练从四个权威 CSV 读取已经记录的字段。
